...
public class VRFLiveLinkTarget : TargetRules
{
	/// <summary>
	...
	public VRFLiveLinkTarget(TargetInfo Target) : base(Target)
	{
		...
		this.LaunchModuleName = "VRFLiveLink";
		...
		string PostBuildBinDir = Path.Combine(DefaultBinDir, "VRFLiveLink");
		if (Target.Platform.ToString() == "Win64")
		{
			...
			//PostBuildSteps.Add(string.Format("xcopy /y /i /v \"{0}\\{1}.*\" \"{2}\" 1>nul", EngineBinariesDir, "sl_VRF_c", PostBuildBinDir));
			...
			//PostBuildSteps.Add(string.Format("xcopy /y /i /v \"{0}\\{1}.*\" \"{2}\" 1>nul", ConfigFilePath, "VRFLiveLinkConfig", PostBuildBinDir));
		}
	}
}